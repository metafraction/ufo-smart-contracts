...
contract Staking is AccessControl, ERC20 {
  ...
  function claimPlasma(address _to) public {
    ...
    uint256 plasmaToGenerate = (
      possiblePlasmaPossible.sub(plasmaClaimedTillNow)
    ).mul(_currentBalance).div(balanceOf(eeee));
    require(
      possiblePlasmaPossible >= plasmaClaimedTillNow.add(plasmaToGenerate),
  ...
  function getLockedDeposit(address _address)
    public
    view
    returns (Locked[] memory)
  {
    ...
    for (uint256 i = 0; i < length; i++) {
      Locked memory lLockedDeposit = lockedDeposit[_address][i];
  ...
  function depositUfoLocked(
    ...
    require(
      _month == 1 || _month == 3 || _month == 9 || _month == 21,
      'month is not valid'
    );
    uint256 weight;
    uint256 withdrawTimestamp = block.timestamp +
      BLOCKS_PER_DAY.mul(86400).mul(30).mul(_month);
    ...
    lockedDeposit[msg.sender].push(
      Locked(_amount, weight, withdrawTimestamp, block.timestamp)
    );
    ...
    onlyAdmin
  {
    ...
    require(
      ...
      'Cannot update plasma points of the past'
    );
    require(
